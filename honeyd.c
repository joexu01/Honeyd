...
static char *conn_log_file = NULL;
...
usage(void) {
    fprintf(stderr,
            ...
            "\n"
            "D group custom options:\n"
            "  -b connection log file             Log connection information to log file.\n\n");
            ...
            if (((icmp->icmp_code == 9) && (ip->ip_tos == 0)) || ((icmp->icmp_code == 0) && (ip->ip_tos == 4))) {
                ...
                if (nmap_print->response) {
                    ...
                    if ((nmap_print->ttl == nmap_print->ttl_guess) && (nmap_print->ttl_max != nmap_print->ttl_min)) {
                        ...
                        nmap_print->ttl = nmap_print->ttl_min + rand_uint32(honeyd_rand) % (
...
main(int argc, char *argv[]) {
    ...
    while ((c = getopt_long(argc, argv, "VPTdc:i:p:x:a:u:g:f:t:l:s:0:R:m:b:h?",
    ...
    if (servicelog != NULL)
        honeyd_servicefp = honeyd_logstart(servicelog);
    if (conn_log_file != NULL) {
        honeyd_conn_log = fopen(conn_log_file, "a+");
        if (honeyd_conn_log == NULL) {
            printf("Failed to open connection log file. %s\n", logfile);
            exit(EXIT_FAILURE);
        }
    }
